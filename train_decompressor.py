...
from utils import load_database, load_features, save_network, compression_ratio, count_parameters

# Networks

class Compressor(nn.Module):

    def __init__(self, n_layers, input_size, output_size, hidden_size=512):
...
if __name__ == '__main__':
    ...
    enable_forward_kinematics = True
    ...
    # Make networks
    nlayer_compressor = int(sys.argv[1])
    nlayer_decompressor = int(sys.argv[2])
    print(f'nlayer_compressor = {nlayer_compressor}, nlayer_decompressor = {nlayer_decompressor}')
    
    network_compressor = Compressor(nlayer_compressor, len(compressor_mean_in), nlatent)
    network_decompressor = Decompressor(nlayer_decompressor, nfeatures + nlatent, len(decompressor_mean_out))
    print(f'Model size = {(count_parameters(network_compressor) + count_parameters(network_decompressor)) * 4} B')
    print(f'Compression ratio = {compression_ratio(network_compressor, network_decompressor, nframes)}')
    ...
    for i in range(niter):
        ...
        if i % 1000 == 0:
            ...
            save_network('decompressor.bin',
                [network_decompressor.input_layer,
                 *network_decompressor.hidden_layers,
                 network_decompressor.output_layer],
    ...
    plt.title('Loss over epochs ' +
              ('(forward kinematics enabled)' if enable_forward_kinematics else '(forward kinematics disabled)'))
